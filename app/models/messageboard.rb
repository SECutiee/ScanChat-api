...
module ScanChat
  ...
  class Messageboard < Sequel::Model
    ...
    many_to_one :thread, key: :thread_id
    ...
    def validate
      super
      errors.add(:thread_id, 'must reference a Thread with threadable_type of messageboard') unless thread_id_valid?
    end

    # Custom validation method to check threadable_type
    def thread_id_valid?
      Thread.where(id: thread_id, threadable_type: 'messageboard').count.positive?
    end
